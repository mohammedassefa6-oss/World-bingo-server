...
app.get('/room-taken',auth,async(req,res)=>{try{const stake=posInt(req.query.stake);if(!ALLOWED_STAKES.has(stake))return res.status(400).json({error:'Invalid stake'});const s=await db.ref(`rooms/stake_${stake}_open/taken`).once('value');res.json({taken:s.val()||{}});}catch(e){console.error(e);res.status(500).json({error:'Could not load room'});}});
...
app.post('/join-room',auth,async(req,res)=>{try{const stake=posInt(req.body.stake),cardNo=posInt(req.body.cartelaNumber);if(!ALLOWED_STAKES.has(stake))return res.status(400).json({error:'Invalid room stake'});if(cardNo===null||cardNo>500)return res.status(400).json({error:'Invalid cartela number'});const roomId=`stake_${stake}_open`,roomRef=db.ref(`rooms/${roomId}`),balRef=db.ref(`users/${req.uid}/balance`);
 ...
 }catch(e){console.error('join-room:',e);res.status(500).json({error:e.message||'Could not joi
 n room'});}});
