...
if (BOT_TOKEN) {
    ...
    bot.start(async (ctx) => {
        try {
            ...
            await ctx.reply(
                `👋 **እንኳን ወደ ቤተሰብ ቢንጎ (Beteseb Bingo) በደህና መጡ!**\n\nጨዋታውን ለመጀመር ወይም አካውንትዎን ለማስተዳደር ከታች ያሉትን አማራጮች ይጠቀሙ:`,
                {
                    parse_mode: 'Markdown',
                    ...Markup.inlineKeyboard([
                        [Markup.button.webApp('🎮 ጨዋታውን ጀምር (Play)', webAppUrl)],
                        [Markup.button.callback('💰 ባላንስ እና ካርቴላ', 'menu_balance'), Markup.button.callback('💳 ገንዘብ ማስገባት', 'menu_deposit')],
                        [Markup.button.callback('💸 ገንዘብ ማውጣት', 'menu_withdraw'), Markup.button.callback('👥 ጓደኛ መጋበዝ', 'menu_invite')],
                        [Markup.button.callback('📞 የደንበኞች እርዳታ (Support)', 'menu_support')]
                    ])
                }
            );
        } catch (e) {
    ...
    bot.action('menu_support', async (ctx) => {
        try { await ctx.answerCbQuery(); } catch (e) {}
        await ctx.reply('📞 **የደንበኞች እርዳታ (Support)**\n\nማንኛውም ጥያቄ ወይም ድጋፍ ካሎት በቀጥታ አድሚኑን ማግኘት ይችላሉ:\n👤 አድሚን: @MohammedAssefa');
    });
...
async function profile(uid) {
    ...
    const p = s.val();
    return p && String(p.telegramId) === uid.slice(3) ? p : null;
}
...
async function auth(req, res, next) {
    ...
    try {
        ...
        const p = await profile(d.uid);
        if (!p) return res.status(403).json({ error: 'Telegram user verification required' });
        req.uid = d.uid;
        req.profile = p;
        next();
    } catch (e) {
...
function adminOnly(req, res, next) {
    auth(req, res, () => {
        if (!ADMIN_UIDS.includes(req.uid)) return res.status(403).json({ error: 'Admin access required' });
        next();
    });
}
...
function verifyTelegram(initData) {
    ...
    const userValue = p.get('user');
    if (!hash || !authDate || !userValue) throw new Error('Invalid Telegram WebApp data');
    p.delete('hash');
    ...
    if (!Number.isFinite(authDate) || age > 300 || age < -30) throw new Error('initData expired, reopen the app');
    let user;
    try { user = JSON.parse(userValue); } catch { throw new Error('Invalid Telegram user data'); }
    if (!user || !user.id) throw new Error('Telegram user is required');
    return { user, startParam: p.get('start_param') || '' };
}

app.get('/health', async (req, res) => {
    try {
        const s = await db.ref('.info/connected').once('value').catch(() => null);
        res.json({ ok: true, service: 'Beteseb Bingo', databaseConfigured: true, connected: s ? s.val() : null });
    } catch (e) {
...
app.post('/verify-telegram-login', async (req, res) => {
    try {
        const { user, startParam } = verifyTelegram(req.body.initData);
        ...
        const pSnap = await userRef.once('value');
        const p = pSnap.val() || {};
        if (!snap.exists() && startParam) {
        ...
        const balance = num((await userRef.child('balance').once('value')).val());
        const freeCards = num((await userRef.child('cards').once('value')).val()) || 0;
        const customToken = await admin.auth().createCustomToken(uid);
        res.json({ customToken, uid, balance: balance === null ? 0 : balance, cards: freeCards, referralCode: p.referralCode || String(user.id) });
    } catch (e) {
...
app.get('/balance', auth, async (req, res) => {
    try {
        let s = await db.ref(`users/${req.uid}/balance`).once('value');
        let b = num(s.val());
        if (b === null) { await db.ref(`users/${req.uid}/balance`).set(0); b = 0; }
        let cSnap = await db.ref(`users/${req.uid}/cards`).once('value');
        let c = num(cSnap.val()) || 0;
        res.json({ balance: b, cards: c });
    } catch (e) {
        console.error(e);
...
app.get('/referral', auth, async (req, res) => {
    ...
    res.json({
        ...
        linkBase: MINI_APP_LINK_BASE
    });
});

app.get('/history', auth, async (req, res) => {
    try {
        const s = await db.ref(`users/${req.uid}/transactions`).orderByChild('createdAt').limitToLast(100).once('value');
        const raw = s.val() || {};
        const items = Object.entries(raw).map(([id, v]) => ({ id, ...v })).sort((a, b) => Number(b.createdAt || 0) - Number(a.createdAt || 0));
        res.json({ items });
    } catch (e) {
...
app.post('/withdrawal-request', auth, async (req, res) => {
    try {
        const amount = money(req.body.amount);
        if (amount === null) return res.status(400).json({ error: 'Invalid withdrawal amount' });
        const balRef = db.ref(`users/${req.uid}/balance`);
        ...
        const id = db.ref('moneyRequests').push().key;
        const request = { uid: req.uid, type: 'withdrawal', amount, status: 'pending', createdAt: admin.database.ServerValue.TIMESTAMP };
        const updates = {};
        updates[`moneyRequests/${id}`] = request;
        updates[`users/${req.uid}/transactions/${id}`] = request;
        try {
        ...
        res.json({ requestId: id, status: 'pending', balance: num(tx.snapshot.val()) || 0 });
    } catch (e) {
        console.error(e);
...
app.post('/join-room', auth, async (req, res) => {
    try {
        const stake = posInt(req.body.stake), cardNo = posInt(req.body.cartelaNumber);
        if (!ALLOWED_STAKES.has(stake)) return res.status(400).json({ error: 'Invalid room stake' });
        if (cardNo === null || cardNo > 500) return res.status(400).json({ error: 'Invalid cartela number' });
        const roomId = `stake_${stake}_open`, roomRef = db.ref(`rooms/${roomId}`), userRef = db.ref(`users/${req.uid}`), balRef = userRef.child('balance'), cardsRef = userRef.child('cards');
        
        let usedFreeCard = false;
        const ctx = await cardsRef.transaction(v => {
            const c = num(v);
        ...
        if (ctx.committed) {
            ...
            if (!btx.committed) return res.status(412).json({ error: `Insufficient balance or free cards.` });
        }
        ...
        const jtx = await roomRef.transaction(room => {
            ...
            if (room.players[req.uid]) return;
            if (room.taken[String(cardNo)]) return;
            room.players[req.uid] = { cartelaNumber: cardNo, joinedAt: Date.now() };
        ...
        if (count >= 2) {
            await roomRef.update({ state: 'running', startedAt: admin.database.ServerValue.TIMESTAMP, calledNumbers: {} });
            room = (await roomRef.once('value')).val();
        }
        const balance = num((await balRef.once('value')).val()) || 0;
        const freeCardsLeft = num((await cardsRef.once('value')).val()) || 0;
        ...
        res.status(500).json({ error: e.message || 'Could not join room' });
    }
});

app.get('/admin/money-requests', adminOnly, async (req, res) => {
    try {
        const s = await db.ref('moneyRequests').orderByChild('createdAt').limitToLast(100).once('value');
        const raw = s.val() || {};
        const items = Object.entries(raw).map(([id, v]) => ({ id, ...v })).sort((a, b) => Number(b.createdAt || 0) - Number(a.createdAt || 0));
        res.json({ items });
    } catch (e) {
...
async function processMoney(req, res, type, status) {
    try {
        const id = String(req.params.id || '');
        const rRef = db.ref(`moneyRequests/${id}`);
        ...
        if (type === 'withdrawal' && status === 'rejected') {
            await db.ref(`users/${r.uid}/balance`).transaction(v => (num(v) || 0) + Number(r.amount));
        }
        const now = admin.database.ServerValue.TIMESTAMP;
        const updates = {};
        updates[`moneyRequests/${id}/status`] = status;
        updates[`moneyRequests/${id}/processedAt`] = now;
        updates[`moneyRequests/${id}/processedBy`] = req.uid;
        updates[`users/${r.uid}/transactions/${id}/status`] = status;
        updates[`users/${r.uid}/transactions/${id}/processedAt`] = now;
        updates[`users/${r.uid}/transactions/${id}/processedBy`] = req.uid;
        await db.ref().update(updates);
        res.json({ ok: true, status, balance: num((await db.ref(`users/${r.uid}/balance`).once('value')).val()) || 0 });
    } catch (e) {
        console.error(e);
...
async function advanceAllRooms() {
    try {
        ...
        console.error('advanceAllRooms:', e.message);
    }
}
...
app.listen(PORT, () => console.log(`Beteseb Bingo listening on ${PORT}`));
