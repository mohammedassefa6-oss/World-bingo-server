...
const DEPOSIT_PHONE=String(process.env.DEPOSIT_PHONE||'');
const DEPOSIT_ACCOUNT_NAME=String(process.env.DEPOSIT_ACCOUNT_NAME||'');
...
async function getActiveStake(uid){try{for(const stake of ALLOWED_STAKES){const s=await db.ref(`rooms/stake_${stake}_open`).once('value');const r=s.val();if(r&&r.players&&r.players[uid]&&(r.state==='waiting'||r.state==='running'))return Number(r.stake)||0;}return 0;}catch(e){return 0;}}
app.get('/balance',auth,async(req,res)=>{try{let s=await db.ref(`users/${req.uid}/balance`).once('value');let b=num(s.val());if(b===null){await db.ref(`users/${req.uid}/balance`).set(0);b=0;}const playWallet=await getActiveStake(req.uid);res.json({balance:b,playWallet});}catch(e){console.error(e);res.status(500).json({error:'Could not load balance'});}});
...
app.get('/config',(req,res)=>{res.json({houseCut:HOUSE_CUT,callIntervalMs:CALL_INTERVAL_MS});});
app.get('/deposit-info',(req,res)=>{res.json({phone:DEPOSIT_PHONE,name:DEPOSIT_ACCOUNT_NAME});});
app.post('/deposit-request',auth,async(req,res)=>{try{const amount=money(req.body.amount);if(amount===null)return res.status(400).json({error:'Invalid deposit amount'});const transactionId=req.body.transactionId?String(req.body.transactionId).slice(0,100):null;const id=db.ref('moneyRequests').push().key;const request={uid:req.uid,type:'deposit',amount,status:'pending',transactionId,createdAt:admin.database.ServerValue.TIMESTAMP};const updates={};updates[`moneyRequests/${id}`]=request;updates[`users/${req.uid}/transactions/${id}`]=request;await db.ref().update(updates);res.json({requestId:id,status:'pending'});}catch(e){console.error(e);res.status(500).json({error:'Could not create deposit request'});}});
...
async function processMoney(req,res,type,status){try{const id=String(req.params.id||'');const rRef=db.ref(`moneyRequests/${id}`);const snap=await rRef.once('value');const r=snap.val();if(!r)return res.status(404).json({error:'Request not found'});if(r.type!==type)return res.status(400).json({error:'Wrong request type'});if(r.status!=='pending')return res.status(409).json({error:'Request already processed'});
 ...
 const verb=status==='approved'?'Approved':'Rejected';const icon=status==='approved'?'✅':'❌';const label=type==='deposit'?'deposit':'withdrawal';
 bot.notifyUser(r.uid,`${icon} Your ${label} of ${r.amount} ETB is ${verb}.\nRef: ${id}`).catch(()=>{});
...
async function advanceAllRooms(){try{const s=await db.ref('rooms').orderByChild('state').equalTo('running').once('value');const rooms=s.val()||{};for(const [roomId,room] of Object.entries(rooms)){const called=new Set(Object.keys(room.calledNumbers||{}).map(Number));const remaining=[];for(let n=1;n<=75;n++)if(!called.has(n))remaining.push(n);if(!remaining.length){await db.ref(`rooms/${roomId}/state`).set('finished');continue;}const next=remaining[Math.floor(Math.random()*remaining.length)];await db.ref(`rooms/${roomId}/calledNumbers/${next}`).set(true);await db.ref(`rooms/${roomId}/lastCalled`).set(next);called.add(next);
 const players=room.players||{};const winners=Object.entries(players).filter(([uid,p])=>hasBingo(p.cartelaNumber,called));
 if(winners.length){const count=Object.keys(players).length;const gross=Number(room.stake)*count;const totalPrize=Math.floor(gross*(1-HOUSE_CUT));const share=Math.floor(totalPrize/winners.length);const roomRef=db.ref(`rooms/${roomId}`);
  const finishTx=await roomRef.transaction(r=>{if(!r||r.state!=='running')return;const winnersObj={};for(const [uid,p] of winners)winnersObj[uid]={cartelaNumber:p.cartelaNumber,prize:share};return {...r,state:'finished',winners:winnersObj,winningNumber:next,payoutStatus:'pending',finishedAt:Date.now()};});
  if(finishTx.committed){for(const [uid] of winners){await db.ref(`users/${uid}/balance`).transaction(v=>(num(v)||0)+share);const txId=db.ref(`users/${uid}/transactions`).push().key;await db.ref(`users/${uid}/transactions/${txId}`).set({type:'win',amount:share,roomId,status:'completed',createdAt:admin.database.ServerValue.TIMESTAMP});}await roomRef.update({payoutStatus:'paid'});}
 }}}catch(e){console.error('advanceAllRooms:',e.message);}}
setInterval(advanceAllRooms,CALL_INTERVAL_MS);

async function tickWaitingRooms(){try{const s=await db.ref('rooms').orderByChild('state').equalTo('waiting').once('value');const rooms=s.val()||{};const now=Date.now();for(const [roomId,room] of Object.entries(rooms)){if(!room.countdownEndsAt||room.countdownEndsAt>now)continue;const roomRef=db.ref(`rooms/${roomId}`);const players=room.players||{};const count=Object.keys(players).length;
 if(count>=2){await roomRef.transaction(r=>{if(!r||r.state!=='waiting')return;return {...r,state:'running',startedAt:Date.now(),calledNumbers:{}};});}
 else{for(const uid of Object.keys(players))await db.ref(`users/${uid}/balance`).transaction(v=>(num(v)||0)+Number(room.stake));await roomRef.set({stake:room.stake,state:'waiting',players:{},taken:{}});}
}}catch(e){console.error('tickWaitingRooms:',e.message);}}
setInterval(tickWaitingRooms,2000);
app.get('*',(req,res)=>res.sendFile(path.join(__dirname,'public','index.html')));
// ---- Telegram bot chat menu (Register/Play/Deposit/Withdraw/Transfer/etc. entirely in chat) ----
const bot=createBot(db,admin);
const WEBHOOK_SECRET=process.env.TELEGRAM_WEBHOOK_SECRET||'';
app.post('/telegram/webhook',(req,res)=>{
  if(WEBHOOK_SECRET){const got=req.headers['x-telegram-bot-api-secret-token'];if(got!==WEBHOOK_SECRET)return res.sendStatus(401);}
  res.sendStatus(200); // ack immediately, Telegram requires a fast response
  bot.handleUpdate(req.body).catch(e=>console.error('handleUpdate:',e));
});

const PORT=Number(process.env.PORT||3000);app.listen(PORT,async()=>{
  console.log(`Beteseb Bingo listening on ${PORT}; Firebase project=${serviceAccount.project_id}`);
  const publicUrl=process.env.PUBLIC_URL||(process.env.RAILWAY_PUBLIC_DOMAIN?`https://${process.env.RAILWAY_PUBLIC_DOMAIN}`:'');
  if(publicUrl) await bot.setWebhook(publicUrl,WEBHOOK_SECRET);
  else console.warn('PUBLIC_URL not set — call setWebhook manually or set PUBLIC_URL / rely on RAILWAY_PUBLIC_DOMAIN.');
  await bot.setCommands().catch(e=>console.error('setCommands failed:',e.message));
});
