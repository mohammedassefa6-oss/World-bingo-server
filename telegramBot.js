...
// If TELEGRAM_MINI_APP_LINK_BASE or PUBLIC_URL is configured,
...
// Play opens the Telegram Mini App.
// The other buttons use callback queries.
...
module.exports = function createBot(db, admin) {
  ...
  async function startDeposit(chatId) {
    ...
    await sendMessage(
      chatId,
      '💵 Enter the amount (ETB) you want to deposit:',
      {
        reply_markup: {
  ...
  async function finishDeposit(
    ...
    if (telebirr.ENABLED) {
      ...
      await sendMessage(
        ...
        `📲 Send ${amount} ETB to our Telebirr number, then paste the <b>Telebirr transaction ID</b> here to get it credited automatically.`
      );
  ...
  async function finishDepositTxnId(
    ...
    if (!transactionId) {
      ...
      await sendMessage(
        chatId,
        '⚠️ Please paste a valid transaction ID.'
      );
    ...
    if (!result.ok) {
      ...
      await sendMessage(
        chatId,
        `⚠️ Could not auto-verify that transaction (${result.reason}). Sending it to admin for manual review instead.`
      );
    ...
    await sendMessage(
      ...
      `✅ ${
        type === 'deposit'
          ? 'Deposit'
          : 'Withdrawal'
      } request sent (ID: ${id}). Waiting for admin approval.`,
  ...
  // Kept here so existing functionality is not removed.
  // Main menu Play now opens the Mini App.
  // ==========================================================
