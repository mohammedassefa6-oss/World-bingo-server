...
const ADMIN_CHAT_ID = process.env.ADMIN_CHAT_ID || ''; // የአድሚን Chat ID ወይም Group ID
...
module.exports = function createBot(db, admin) {
  ...
  async function applyReferral(uid, startParam) {
    ...
    if (REFERRAL_BONUS) {
      await db.ref(`users/${refUid}/bonus`).transaction(v => (Number(v) || 0) + REFERRAL_BONUS);
    }
  ...
  async function getActiveStake(uid) {
    for (const stake of ALLOWED_STAKES) {
      const s = await db.ref(`rooms/stake_${stake}_open`).once('value');
      const r = s.val();
  ...
  async function handleBalance(chatId, uid) {
    ...
    await sendMessage(
      chatId,
      `💰 <b>Main Wallet:</b> ${Number(u.balance || 0)} ETB\n` +
  ...
  // ==========================================================
  // INSTRUCTION & SUPPORT & INVITE & BONUS
  // ==========================================================
  ...
  async function handleConvertBonus(chatId, uid) {
    ...
    const snap = await bonusRef.once('value');
    const bonus = Number(snap.val() || 0);

    if (bonus <= 0) {
      await sendMessage(chatId, '🎉 No bonus available to convert right now.', { reply_markup: mainMenuKeyboard() });
      return;
    }

    const tx = await bonusRef.transaction(v => 0);
    if (!tx.committed) {
      await sendMessage(chatId, '⚠️ Could not convert bonus, try again.', { reply_markup: mainMenuKeyboard() });
      return;
    }

    await db.ref(`users/${uid}/balance`).transaction(v => Math.round(((Number(v) || 0) + bonus) * 100) / 100);

    await sendMessage(chatId, `🎉 Converted ${bonus} ETB bonus into your balance!`, { reply_markup: mainMenuKeyboard() });
  }
  ...
  async function finishDeposit(chatId, uid, text) {
    ...
    if (amount === null) {
      await sendMessage(chatId, '⚠️ Invalid amount, please enter a number, e.g. 100');
      return;
    }
  ...
  async function finishDepositTxnId(chatId, uid, text) {
    ...
    const amount = s.depositAmount;
    const transactionId = String(text).trim();

    if (!transactionId) {
      await sendMessage(chatId, '⚠️ Please paste a valid transaction ID.');
      return;
    }
    ...
    if (!claim.committed) {
      clearSession(chatId);
      await sendMessage(chatId, '⚠️ This transaction ID has already been used.', { reply_markup: mainMenuKeyboard() });
      return;
    }
    ...
    await db.ref(`users/${uid}/transactions/${txId}`).set({
      type: 'deposit',
      amount,
      status: 'completed',
      provider: 'telebirr',
  ...
  async function finishWithdraw(chatId, uid, text) {
    ...
    const tx = await balRef.transaction(v => {
      const b = num(v);
      if (b === null || b < amount) return;
      return Math.round((b - amount) * 100) / 100;
    });

    if (!tx.committed) {
      await sendMessage(chatId, '⚠️ Insufficient balance.');
      clearSession(chatId);
      await sendMessage(chatId, 'Menu:', { reply_markup: mainMenuKeyboard() });
      return;
    }

    if (telebirr.ENABLED) {
      const uSnap = await db.ref(`users/${uid}`).once('value');
      const phone = (uSnap.val() || {}).phone;
  ...
  async function createManualMoneyRequest(
    ...
    await db.ref().update({
      [`moneyRequests/${id}`]: request,
      [`users/${uid}/transactions/${id}`]: request
    });
    ...
    await sendMessage(
      chatId,
      `✅ ${type === 'deposit' ? 'Deposit' : 'Withdrawal'} request sent (ID: ${id}). Waiting for admin approval.`,
      { reply_markup: mainMenuKeyboard() }
    );

    // ለአድሚን ማሳወቂያ እና [✅ Approve] / [❌ Reject] ቁልፍ መላክ
    if (ADMIN_CHAT_ID) {
      const adminText = 
      ...
      const adminKeyboard = {
        inline_keyboard: [
          [
            { text: '✅ Approve', callback_data: `admin:approve:${id}` },
            { text: '❌ Reject', callback_data: `admin:reject:${id}` }
          ]
        ]
      };

      await sendMessage(ADMIN_CHAT_ID, adminText, { reply_markup: adminKeyboard });
    }
  }
  ...
  async function transferRecipient(chatId, text) {
    ...
    if (!/^\d+$/.test(targetTelegramId)) {
      await sendMessage(chatId, '⚠️ Please send a numeric Telegram ID.');
      return;
    }
    ...
    if (!exists.exists()) {
      await sendMessage(chatId, '⚠️ No user found with that Telegram ID.');
      return;
    }
  ...
  async function finishTransfer(chatId, uid, text) {
    const s = getSession(chatId);
    const targetUid = s.transferTarget;
    const amount = money(text);
    ...
    if (targetUid === uid) {
      await sendMessage(chatId, '⚠️ You cannot transfer to yourself.');
      clearSession(chatId);
      return;
    }
  ...
  async function showMiniApp(chatId) {
    await sendMessage(
      chatId,
      '🎱 <b>Beteseb Bingo</b>\n\n' +
      '🎮 የBingo ጨዋታውን ለመክፈት <b>Play Bingo</b> ይጫኑ።',
      {
        reply_markup: {
          inline_keyboard: [
  ...
  async function showCardGrid(chatId, messageId, stake, page) {
    ...
    if (messageId) {
      await editMessage(chatId, messageId, text, { reply_markup: kb });
    } else {
  ...
  async function joinRoom(chatId, uid, stake, cardNo) {
    ...
    const btx = await balRef.transaction(v => {
      const b = num(v);
      if (b === null || b < stake) return;
      return Math.round((b - stake) * 100) / 100;
    });

    if (!btx.committed) {
      await sendMessage(chatId, '⚠️ Insufficient balance for this stake.');
      return;
    }
    ...
    const jtx = await roomRef.transaction(room => {
      ...
      if (room.players[uid] || room.taken[String(cardNo)]) return;

      room.players[uid] = { cartelaNumber: cardNo, joinedAt: Date.now() };
      room.taken[String(cardNo)] = true;

      if (!room.countdownEndsAt) {
        room.countdownEndsAt = Date.now() + CARTELA_SELECTION_SECONDS * 1000;
      }
      return room;
    });

    if (!jtx.committed) {
      await balRef.transaction(v => (num(v) || 0) + stake);
      await sendMessage(chatId, '⚠️ That cartela is already taken, pick another.');
      return;
    }
  ...
  function renderGameText(card, calledArr, lastCalled) {
    ...
    for (let r = 0; r < 5; r++) {
      grid += cols.map(c => String(mark(card[c][r])).padEnd(4)).join('') + '\n';
    }
  ...
  function startGameWatcher(chatId, uid, roomId, cardNo, card, messageId) {
    ...
    const intervalId = setInterval(async () => {
      ...
      if (room.state === 'waiting') {
        if (messageId) {
          await editMessage(chatId, messageId, renderWaitingText(room), { reply_markup: gameKeyboard() }).catch(() => {});
        }
        return;
      }
      ...
      if (messageId) {
        await editMessage(chatId, messageId, renderGameText(card, calledArr, room.lastCalled), { reply_markup: gameKeyboard() }).catch(() => {});
      }
  ...
  async function claimBingo(chatId, uid) {
    const w = watchers.get(chatId);
    if (!w) {
      await sendMessage(chatId, '⚠️ You are not in an active game.');
      return;
    }
    ...
    if (!room || room.state !== 'running') {
      await sendMessage(chatId, '⚠️ Room not active.');
      return;
    }

    const called = new Set(Object.keys(room.calledNumbers || {}).map(Number));
    if (!hasBingo(w.cardNo, called)) {
      await sendMessage(chatId, '⚠️ No BINGO on your card yet.');
      return;
    }
    ...
    const claimTx = await roomRef.transaction(r => {
      if (!r || r.state !== 'running' || !r.players || !r.players[uid]) return;
      return {
        ...r,
        state: 'finished',
        winners: { [uid]: { cartelaNumber: w.cardNo, prize } },
        winningNumber: room.lastCalled,
    ...
    if (!claimTx.committed) {
      await sendMessage(chatId, '⚠️ This round has already been claimed.');
      return;
    }
  ...
  async function handleUpdate(update) {
    try {
      if (update.callback_query) {
        return await onCallback(update.callback_query);
      }
      ...
      console.error('bot handleUpdate:', e);
    }
  }
  ...
  async function onMessage(msg) {
    ...
    if (msg.contact) {
      return handleRegister(chatId, uid, msg.contact);
    }
    ...
    if (session.awaiting === 'register_contact') return;
    ...
    switch (text) {
      ...
      case MENU_TEXT.register:
        return handleRegister(chatId, uid, null);
      case MENU_TEXT.balance:
        return handleBalance(chatId, uid);
      ...
      case MENU_TEXT.invite:
        return handleInvite(chatId, uid);
      case MENU_TEXT.bonus:
        return handleConvertBonus(chatId, uid);
      default:
        await sendMessage(
          chatId,
  ...
  async function onCallback(cq) {
    ...
    if (data.startsWith('admin:')) {
      ...
      const reqRef = db.ref(`moneyRequests/${requestId}`);
      const reqSnap = await reqRef.once('value');

      if (!reqSnap.exists()) {
        await sendMessage(chatId, '⚠️ Request not found.');
        return;
      }
      ...
      if (req.status !== 'pending') {
        await sendMessage(chatId, `⚠️ Request is already ${req.status}.`);
        return;
      }
      ...
      if (action === 'approve') {
        // Approve logic
        if (req.type === 'deposit') {
          await db.ref(`users/${req.uid}/balance`).transaction(
            v => Math.round(((Number(v) || 0) + req.amount) * 100) / 100
          );
        }

        await reqRef.update({ status: 'approved' });
        await db.ref(`users/${req.uid}/transactions/${requestId}`).update({ status: 'approved' });
        ...
        await notifyUser(
          req.uid,
          `✅ Your ${req.type} request of ${req.amount} ETB has been APPROVED!`
        );

      } else if (action === 'reject') {
        // Reject logic
        if (req.type === 'withdrawal') {
          // Return deducted amount back to user's balance
          await db.ref(`users/${req.uid}/balance`).transaction(
            v => Math.round(((Number(v) || 0) + req.amount) * 100) / 100
          );
        }
        ...
        await db.ref(`users/${req.uid}/transactions/${requestId}`).update({ status: 'rejected' });
        ...
        await notifyUser(
          req.uid,
          `❌ Your ${req.type} request of ${req.amount} ETB was REJECTED.`
        );
      }
    ...
    const menuActions = {
      'menu:register': () => handleRegister(chatId, uid, null),
      'menu:balance': () => handleBalance(chatId, uid),
      'menu:deposit': () => startDeposit(chatId),
      'menu:support': () => handleSupport(chatId),
      'menu:instruction': () => handleInstruction(chatId),
      'menu:transfer': () => startTransfer(chatId),
      'menu:withdraw': () => startWithdraw(chatId),
      'menu:invite': () => handleInvite(chatId, uid),
      'menu:bonus': () => handleConvertBonus(chatId, uid)
    };
    ...
    if (data.startsWith('stake:')) {
      ...
      if (!ALLOWED_STAKES.includes(stake)) {
        return answerCallback(cq.id, 'Invalid stake');
      }
      await answerCallback(cq.id);
    ...
    if (data === 'bingo') {
      await answerCallback(cq.id, 'Checking...');
      return claimBingo(chatId, uid);
    }

    if (data === 'leave') {
      stopGameWatcher(chatId);
      await answerCallback(cq.id, 'Left the game');
      return sendMessage(chatId, 'Menu:', { reply_markup: mainMenuKeyboard() });
    }
  ...
  // ==========================================================
  // WEBHOOK & NOTIFY USER & BOT COMMANDS
  // ==========================================================

  async function setWebhook(publicUrl, secretToken) {
    if (!publicUrl) {
      console.warn('setWebhook skipped: no public URL provided');
      return;
    }
  ...
  const BOT_COMMANDS = [
    ...
    { command: 'play', description: 'Open Bingo Mini App' },
  ...
  async function setCommands() {
    return tgCall('setMyCommands', { commands: BOT_COMMANDS });
  }
